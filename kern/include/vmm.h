...
#define rbn2vma(node, member)   to_struct((node), struct vma_struct, member)
...
struct mm_struct {
    ...
    // redblack tree link which sorted by start addr of vma
    rb_tree *mmap_tree;
    // current accessed vma, used for speed purpose
    struct vma_struct *mmap_cache;
    // the PDT of these vma
    pde_t *pgdir;
    // the count of these vma
    int map_count;
};

// If the count of vma >32 then redblack tree link is used.
