...
/**
 * The buddy memory allocation technique is a memory allocation algorithm that
 * divides memory into partitions to try to satisfy a memory request as suitably
 * as possible. This system makes use of splitting memory into halves to try to
 * give a best-fit.
 *
 * The "Buddy System" algorithm in ucore is drived from:
 * 1 Page 203~206, Section 8.4 of Yan Wei Ming's chinese book "Data Structure --
 *   C programming language"
 * 2 Algorithm R & S of section 2.5 of Volume 1 of Knuth's "The Art of Computer
 *   Programming", with note taken of exercises 25 and 29 of that section.
 *
 * In a buddy system, the entire memory space available for allocation is
 * initially treated as a single block whose size is a power of 2. When the
 * first request is made, if its size is greater than half of the initial block
 * then the entire block is allocated. Otherwise, the block is split in two
 * equal companion buddies. If the size of the request is greater than half of
 * one of the buddies, then allocate one to it. Otherwise, one of the buddies is
 * split in half again. This method continues until the smallest block greater
 * than or equal to the size of the request is found and allocated to it.
 *
 * In this method, when a process terminates the buddy block that was allocated
 * to it is freed. Whenever possible, an unnallocated buddy is merged with a
 * companion buddy in order to form a larger free block. Two blocks are said to
 * be companion buddies if they resulted from the split of the same direct
 * parent block.
 */
...
// page2idx - get the related index number idx of continuous page block which
...
// idx2page - get the related page according to the index number idx of
//            continuous page block
static inline struct Page *
